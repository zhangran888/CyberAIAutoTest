...
public class MyExtentTestNgFormatter extends ExtentTestNgFormatter {
    ...
    public MyExtentTestNgFormatter() {
        ...
        // reportPath 报告路径
        String reportPathStr = System.getProperty("user.dir")+"/"+OUTPUT_FOLDER;
        File reportPath;
        ...
        //  报告名report.html
        File reportFile = new File(reportPath, FILE_NAME);
...
//        EmailReporter emailReporter = new EmailReporter(emailReportFile);
        ...
        htmlReporter.config().setDocumentTitle(PropertyUtils.getProperty("testAppName")+"接口自动化测试报告");
        htmlReporter.config().setReportName(PropertyUtils.getProperty("testAppName")+"接口自动化测试报告");
...
//            ExtentTest test = testContext.createNode(iTestResult.getName(), iInvokedMethod.getTestMethod().getDescription());
            iTestResult.setAttribute("test", testContext);
        }
    }
    ...
    private void addNewNode(String parent, String nodeName) {
        ...
        System.out.println("t2:"+nodeName);
